...
MRichTextEdit::MRichTextEdit(QWidget *parent) : QWidget(parent) {
    ...
    connect(textBrowser, SIGNAL(anchorClicked(QUrl)), this, SLOT(openUrl(QUrl)));
}
...
void MRichTextEdit::textSource() {
    ...
    textBrowser->setHtml(f_textedit->toHtml());

    delete dialog;
}

void MRichTextEdit::openUrl(const QUrl& url) const
{
    qDebug() << url.url();
}
